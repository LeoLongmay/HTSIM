...
| Eval (vs median CC) | `expA_delaydriven/` (MNSCC arm, figA2dd_signal); `expB_oversub_asym/` (4:1); `expD_scale1024/` (D1, D2) | PRISM floor beats MNSCC median under asymmetry/load (+11–25% at expA; +6–38% goodput at expB 4:1; −2.3% PRISM loss at D2 4:1 f1 single cell, win emerges cleanly at f3–f4 +16–26%); generalizes to 1024-node D1 (+6–20%) and D2 4:1 (floor beats at f3–f4); median avoids PRISM's f0 cost. **Honest caveat: one cell loss — at D2 4:1 f1 REPS+MNSCC beats PRISM by 2.3% (974.9 vs 952.7 Gbps); do not claim a blanket win at 4:1 1024-node scale.** |
| Eval (vs Swift) | `expA_delaydriven/` (Swift arm); `expB_oversub_asym/` (4:1); `expD_scale1024/` (D1, D2) | PRISM floor beats Swift delay-AIMD (+13% f8, +36% load at expA); win extends to expB 4:1 (+17–23% goodput; at f0 PRISM has small goodput cost but faster FCT); extends to 1024-node D1 (+6–25% at f≥8) and D2 4:1 (+24–34%); D2 8:1 Swift graceful-degrades better than NSCC (cr 0.53 vs 0.36) but still trails PRISM (cr 0.84); Swift ~ REPS/STrack/MNSCC cluster in expA |
| Eval (vs MSwift) | `expA_delaydriven/` (MSwift arm); `expB_oversub_asym/` (4:1); `expD_scale1024/` (D1, D2) | PRISM floor beats even the paper's strongest CC (MSwift) by the narrowest margin (+5–7% f≥4, +6.7–18.5% load at expA); win extends to expB 4:1 (+8.5–10.2%; MSwift = strongest new challenger at expB, cr=1.00 everywhere); extends to 1024-node D1 (+2.6–6.2% — narrowest gap of any arm at scale, honest softener); D2 4:1 (+11–15%); D2 8:1 MSwift graceful-degrades to cr 0.66 vs PRISM cr 0.84; MSwift = strongest non-PRISM arm across all scales |
| Eval (win, load) | `expA_delaydriven/` (figA3dd_load); `expD_scale1024/` (figD3_load, failed=16, 1:1, ρ rel. 6.4 Tbps) | open-loop offered-load sweep: PRISM's FCT lead widens with load under asymmetry (expA). Win reproduces at 8× scale (1024 nodes, all 175 cells cr=1.00): goodput lead is load-dependent and peaks at ρ=0.7 (+16.4% vs REPS+NSCC, +41.4% vs REPS+Swift, +18.0% vs STrack); avg-FCT at ρ=0.5 is 1.8× lower than REPS+NSCC and 3.6× lower than REPS+Swift. **Honest caveat:** at ρ=0.1 (near-idle) PRISM vs REPS+MSwift is −0.4% — a near-idle tie/slight-loss consistent with "the win emerges with load"; do not claim a blanket win at every load point. |
