#!/usr/bin/env python3
"""Run the locked ExpJ DecMT component ablation through ``run_lib.sh``."""
...
COMMON = DATACENTER / "prism_eval" / "common"
...
ARMS = {
    ...
    "floor_only": ("prism", "reps", "14", "-prism_floor_only"),
    "decmt": ("prism", "reps", "14", None),
}
FAILED_LINKS = (0, 8)
SEEDS = tuple(range(13, 23))
SMOKE_FAILED = 8
SMOKE_SEED = 13
PATHS, END_MS, NODES, MTU = 8, 8, 128, 4150
...
class Case:
    """One fixed arm, throttled-link setting, and RNG seed."""
...
def _validate_case(case: Case, *, phase: str | None = None) -> Case:
    if case.arm not in ARMS or case.seed not in SEEDS:
        raise ValueError("case must use a locked ExpJ arm and seed")
    allowed_failed = (SMOKE_FAILED,) if phase == "smoke" else FAILED_LINKS
    if case.failed not in allowed_failed:
        raise ValueError("case must use a locked ExpJ failed-link count")
    return case
...
def cases_for_phase(phase: str) -> tuple[Case, ...]:
    """Return the stable, locked case matrix for ``phase``."""
    if phase == "formal":
...
def case_id(case: Case) -> str:
    return f"expj_{case.arm}_f{case.failed}_s{case.seed}"
...
def ensure_workload(path: Path) -> str:
    """Create exactly the locked 64-flow workload or reject a conflicting one."""
    expected = _workload_text()
    if path.exists():
        if not path.is_file() or path.read_text(encoding="ascii") != expected:
            raise ValueError(f"conflicting ExpJ workload: {path}")
    else:
...
def fixed_environment(case: Case) -> dict[str, str]:
    """Return an isolated environment containing only the fixed runner knobs."""
    _validate_case(case)
    ...
    if arm_extra is not None:
        extra_args.append(arm_extra)
    env.update({
        "PATHS": str(PATHS),
        "END_MS": str(END_MS),
        "MTU": str(MTU),
    ...
    if target_q_delay is not None:
        env["TQD"] = target_q_delay
    return env
...
def _scan_flow_events(path: Path) -> set[tuple[int, int]]:
    ...
    if any(finishes[key] < start for key, start in starts.items()):
...
def _flow_event_bindings(
    event_keys: set[tuple[int, int]], idmap: dict[int, tuple[int, int]]
...
def _serialized_event_bindings(bindings: dict[tuple[int, int], tuple[int, int]]) -> list[dict]:
    return [
        {
            "src_id": event_key[0], "flow_id": event_key[1],
            "workload_src": endpoint[0], "workload_dst": endpoint[1],
        }
...
def _expected_manifest(
    *, phase: str, case: Case, workload: Path, workload_hash: str, output_root: Path
) -> dict:
    run_id = case_id(case)
    _, _, target_q_delay, arm_extra = ARMS[case.arm]
    ...
    return {
        ...
        "fixed_parameters": {
            ...
            "mtu": MTU,
            "disable_trim": True,
            "target_q_delay_us": int(target_q_delay) if target_q_delay is not None else None,
            "arm_extra_args": arm_extra,
        ...
        "config": {
            ...
            } | ({"TQD": target_q_delay} if target_q_delay is not None else {}),
        },
...
def run_one(case: Case, *, phase: str, output_root: Path) -> Path:
    """Run one validated case, or reuse only its exact immutable output bundle."""
    ...
    if not all(path.is_file() for path in (TOPOLOGY, HTSIM_UEC, PARSE_OUTPUT, RUN_LIB)):
        raise FileNotFoundError("ExpJ topology, htsim_uec, parse_output, or run_lib is missing")
    subprocess.run(
