...
records across all sinks in each 10-us bin.  The sink logger emits byte/s, so
analysis converts each record to Gb/s with `bytes/s * 8 / 1e9`.  It is
controller-neutral and is **not a direct packet-departure trace**.  The plotted
median is smoothed only for display by a centered 50-us arithmetic mean;
stability statistics always use the unsmoothed aggregate bins.
...
```text
many2many.py 64 16 pairs 32000000 128 16
```
