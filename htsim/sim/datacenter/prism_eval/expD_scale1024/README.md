...
Prism is the **most fair among the NSCC-family arms** at failed ≥ 8 (e.g. f32: 0.941 vs REPS+NSCC 0.910 / STrack 0.913 / REPS+MNSCC 0.909; f40: 0.960 vs REPS+NSCC 0.915 / STrack 0.917 / REPS+MNSCC 0.919). The Swift-family arms (REPS+Swift, REPS+MSwift) achieve higher fairness indices than Prism at moderate asymmetry — e.g. f32: MSwift 0.969, Swift 0.956, Prism 0.941. At f40 Prism (0.960) nearly closes this gap — it trails REPS+MSwift by 0.004 and REPS+Swift by 0.008, both within noise. Notably, Prism's f40 fairness (0.960) is a local peak above both its f32 (0.941) and f48 (0.944) neighbors; the other arms show a similar smaller recovery around this failure level, consistent with heavy asymmetry at 62.5% pod0 choke producing a distinctive flow-balance regime at that point. This is consistent with their rate-reduction behaviour under asymmetry (more conservative sends → more uniform across flows), but does not translate to better goodput or FCT. At f0 Prism (0.898) is slightly less fair than REPS+NSCC (0.932), same under-growth cost as for goodput.
