...
usage() {
  echo "usage: $0 [--all-baselines] [--dry-run]" >&2
}
...
emit_cell() {
  ...
  printf "PATHS=8 END_MS=8 EXTRA_ARGS='%s' bash %s %s %s %s %s %s %s flow expL_%s_f%s_s%s %s\n" \
    "$LOSSLESS_ARGS" "$COMMON/run_lib.sh" "$cc" "$lb" "$failed" "$TOPO" "$seed" \
...
for failed in "${FAILEDS[@]}"; do
  for seed in "${SEEDS[@]}"; do
    for arm in "${BASELINES[@]}"; do
      read -r cc lb <<<"$(arm_config "$arm")"
      PATHS=8 END_MS=8 EXTRA_ARGS="$LOSSLESS_ARGS" \
