...
OUT=mvp_runs3
echo "[run_one] lb=$LB failed=$FAILED tag=$TAG cm=$CM paths=$PATHS end=$END seed=$SEED${TQD:+ tqd=$TQD}"
PRISM_PATHRTT="$OUT/$TAG.pathrtt.csv" ./htsim_uec \
    ...
    -paths "$PATHS" -seed "$SEED" $TQD_ARG -end "$END" -o "$OUT/$TAG.dat" > "$OUT/$TAG.stdout" 2>&1
