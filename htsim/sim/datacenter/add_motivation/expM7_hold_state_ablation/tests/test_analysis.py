"""TDD coverage for M7 finite-flow aggregation and rendering."""
...
from htsim.sim.datacenter.add_motivation.expM7_hold_state_ablation import analyzer, make_figs, run


class M7AnalysisTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_analyze_writes_exact_matrix_and_ten_seed_means(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (aggregate / "m7_per_seed.csv").open(newline="", encoding="ascii") as stream:
                per_seed = list(csv.DictReader(stream))
            with (aggregate / "m7_summary.csv").open(newline="", encoding="ascii") as stream:
            ...
            summary_row = next(row for row in summary if row["arm"] == "observe_only_prism" and row["failed_links"] == "8")
    ...
    def test_analyze_rejects_missing_or_runner_invalid_input(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            _write_formal_fixture(formal)
            (formal / "m7_reps_nscc_f0_s13.manifest.json").unlink()
        ...
        with tempfile.TemporaryDirectory() as directory:
            formal = Path(directory) / "formal"
            _write_formal_fixture(formal)
            (formal / "m7_reps_nscc_f0_s13.idmap").write_text("1 Uec_16_0\n", encoding="ascii")
    ...
    def test_figure_reads_summary_only_and_writes_single_stem(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            pdf = make_figs.render_figure(aggregate / "m7_summary.csv", root / "figs")

            self.assertEqual(pdf.name, "m7_hold_state_ablation.pdf")
            self.assertGreater(pdf.stat().st_size, 0)
            self.assertGreater((root / "figs" / "m7_hold_state_ablation.png").stat().st_size, 0)
    ...
    def test_figure_rejects_nonfinite_summary_value(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            _write_formal_fixture(formal)
            analyzer.analyze_formal(formal, aggregate)
            summary_path = aggregate / "m7_summary.csv"
