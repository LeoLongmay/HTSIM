#!/usr/bin/env python3
"""Render the fixed M7 Prism hold/state-machine ablation from summary CSV only."""
...
ARMS = (
    ...
    ("hold_to_increase_prism", "Hold-to-Increase Prism", plot_style.COLORS["strack"]),
    ("observe_only_prism", "Observe-only Prism", plot_style.COLORS["swift"]),
)
FAILED_LINKS = (0, 4, 8)
...
def render_figure(summary_path: Path, figures_root: Path) -> Path:
    ...
    stem = figures_root / "m7_hold_state_ablation"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--summary", type=Path, default=HERE / "data" / "aggregate" / "m7_summary.csv")
