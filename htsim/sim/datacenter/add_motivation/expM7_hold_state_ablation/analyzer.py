#!/usr/bin/env python3
"""Validate and aggregate the locked 120-case M7 finite-flow experiment."""
...
if __package__ in (None, ""):
    ...
    from htsim.sim.datacenter.add_motivation.expM7_hold_state_ablation import run
else:
    from . import run
...
def _load_case(manifest_path: Path, data_root: Path) -> dict:
    try:
        ...
        raise ValueError(f"invalid M7 manifest identity: {manifest_path}") from exc
    ...
    try:
        ...
        raise ValueError(f"invalid M7 runner output for {run_id}: {exc}") from exc
...
def analyze_formal(data_root: Path, output_root: Path) -> dict[str, list[dict]]:
    ...
    _write_csv(Path(output_root) / "m7_per_seed.csv", PER_SEED_FIELDS, per_seed)
    _write_csv(Path(output_root) / "m7_summary.csv", SUMMARY_FIELDS, summary)
