#!/usr/bin/env python3
"""Run the locked M6 path-robust Prism experiment through run_lib.sh."""
...
SCHEMA = "m6_path_robust_signal_runner"
SCHEMA_VERSION = 1
ARMS = {
    "reps_nscc": ("nscc", "reps_actual", None, None),
    "original_prism": ("prism", "reps_actual", "original_prism", None),
    "path_robust_spread_prism": ("prism", "reps_actual", "original_prism", "spread"),
}
FAILED_LINKS = (0, 4, 8)
...
def _validate_case(case: Case, *, phase: str | None = None) -> Case:
    if case.arm not in ARMS or case.seed not in SEEDS:
        raise ValueError("case must use a locked M6 arm and seed")
    allowed_failed = (SMOKE_FAILED,) if phase == "smoke" else FAILED_LINKS
    if case.failed not in allowed_failed:
        raise ValueError("case must use a locked M6 failed-link count")
    return case
...
def case_id(case: Case) -> str:
    return f"m6_{case.arm}_f{case.failed}_s{case.seed}"
...
def ensure_workload(path: Path) -> str:
    ...
    if path.exists():
        if not path.is_file() or path.read_text(encoding="ascii") != expected:
            raise ValueError(f"conflicting M6 workload: {path}")
    else:
...
def fixed_environment(case: Case) -> dict[str, str]:
    cc, _, coordination_mode, signal_mode = ARMS[case.arm]
    del cc
    env = os.environ.copy()
    for name in (
    ...
    if signal_mode == "full":
        extra_args.append("-prism_path_median_signal")
    elif signal_mode == "spread":
        extra_args.append("-prism_path_median_spread")
    env.update(
        {
            "PATHS": str(PATHS),
            "END_MS": str(END_MS),
...
def _expected_manifest(*, phase: str, case: Case, workload: Path, workload_hash: str,
    ...
    return {
        ...
        "experiment": "M6_path_robust_signal",
        "phase": phase,
        "run_id": run_id,
        "arm": case.arm,
...
def run_one(case: Case, *, phase: str, output_root: Path) -> Path:
    ...
    if not all(path.is_file() for path in (TOPOLOGY, HTSIM_UEC, PARSE_OUTPUT, RUN_LIB)):
        raise FileNotFoundError("M6 topology, htsim_uec, parse_output, or run_lib is missing")
    subprocess.run(
