#!/usr/bin/env python3
"""Validate and aggregate the locked 90-case M6 finite-flow experiment."""
...
def _expected_manifest_paths(data_root: Path) -> dict[tuple[str, int, int], Path]:
    ...
    if actual != set(expected.values()):
        ...
        raise ValueError(f"formal data must contain exactly 90 manifests; missing={missing}, unexpected={unexpected}")
    return expected
...
def _load_case(manifest_path: Path, data_root: Path) -> dict:
    try:
        ...
        raise ValueError(f"invalid M6 manifest identity: {manifest_path}") from exc
    ...
    try:
        ...
        raise ValueError(f"invalid M6 runner output for {run_id}: {exc}") from exc
...
def analyze_formal(data_root: Path, output_root: Path) -> dict[str, list[dict]]:
    ...
    _write_csv(Path(output_root) / "m6_per_seed.csv", PER_SEED_FIELDS, per_seed)
    _write_csv(Path(output_root) / "m6_summary.csv", SUMMARY_FIELDS, summary)
