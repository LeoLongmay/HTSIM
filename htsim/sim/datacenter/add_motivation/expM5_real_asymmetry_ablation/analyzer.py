#!/usr/bin/env python3
"""Validate and aggregate the locked 120-case M5 finite-flow experiment."""
...
if __package__ in (None, ""):
    ...
    from htsim.sim.datacenter.add_motivation.expM5_real_asymmetry_ablation import run
else:
    from . import run
...
def _expected_manifest_paths(data_root: Path) -> dict[tuple[str, int, int], Path]:
    expected = {
        (case.arm, case.failed, case.seed): data_root / f"{run.case_id(case)}.manifest.json"
        for case in run.cases_for_phase("formal")
    }
    actual = set(data_root.glob("*.manifest.json"))
    if actual != set(expected.values()):
        missing = sorted(path.name for path in set(expected.values()) - actual)
        unexpected = sorted(path.name for path in actual - set(expected.values()))
        raise ValueError(f"formal data must contain exactly 120 manifests; missing={missing}, unexpected={unexpected}")
    return expected
...
def _load_case(manifest_path: Path, data_root: Path) -> dict:
    try:
        ...
        case = run.Case(manifest["arm"], int(manifest["failed_links"]), int(manifest["seed"]))
        run._validate_case(case, phase="formal")
    except (OSError, UnicodeDecodeError, json.JSONDecodeError, KeyError, TypeError, ValueError) as exc:
        raise ValueError(f"invalid M5 manifest identity: {manifest_path}") from exc
    run_id = run.case_id(case)
    if manifest_path.name != f"{run_id}.manifest.json":
        raise ValueError(f"manifest filename conflicts with identity: {manifest_path}")
    workload = data_root / "m2m.cm"
    try:
        workload_text = workload.read_text(encoding="ascii")
    except (OSError, UnicodeDecodeError) as exc:
        raise ValueError(f"invalid workload for {run_id}") from exc
    if workload_text != run._workload_text():
        raise ValueError(f"invalid workload for {run_id}")
    outputs = run._required_outputs(data_root, run_id)
    ...
    try:
        if not run._reuse_or_reject(outputs, expected):
            raise ValueError("missing required output")
    except (OSError, ValueError) as exc:
        raise ValueError(f"invalid M5 runner output for {run_id}: {exc}") from exc
    flow_path = outputs["flow"]
    stats = fct_stats(flow_path)
    if (
        stats["completion_rate"] != 1.0
        or stats["completed"] != run.FLOW_COUNT
        or stats["total_started"] != run.FLOW_COUNT
    ):
...
def _summary(per_seed: list[dict]) -> list[dict]:
    ...
    for arm in run.ARMS:
        for failed in run.FAILED_LINKS:
            rows = [row for row in per_seed if row["arm"] == arm and row["failed_links"] == failed]
            if len(rows) != len(run.SEEDS) or {row["seed"] for row in rows} != set(run.SEEDS):
                raise ValueError(f"missing seeds for {arm}/failed={failed}")
            values = {
                field: [row[field] for row in rows]
                for field in ("avg_fct_us", "p99_fct_us", "goodput_gbps", "completion_rate")
            }
            summary.append({
                "arm": arm,
                "failed_links": failed,
...
def analyze_formal(data_root: Path, output_root: Path) -> dict[str, list[dict]]:
    ...
    _write_csv(Path(output_root) / "m5_per_seed.csv", PER_SEED_FIELDS, per_seed)
    _write_csv(Path(output_root) / "m5_summary.csv", SUMMARY_FIELDS, summary)
